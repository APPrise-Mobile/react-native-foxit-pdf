...
#define _isiOS7 (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0"))
...
const CGFloat kTSAlertView_LeftMarginiOS7 = 10;
...
- (void) setStyle:(TSAlertViewStyle)newStyle
{
	if ( style != newStyle )
	{
		...
		if ( style == TSAlertViewStyleInputText || style == TSAlertViewStyleInputNumber )
		{
...
{
	if ( (self.style == TSAlertViewStyleInputText || self.style == TSAlertViewStyleInputNumber) && [self.inputTextField isFirstResponder] )
	{
        float osVerison =[[[UIDevice currentDevice] systemVersion] floatValue] ;
...
//    [DEMO_APPDELEGATE.app unregisterAppPasswordEvent:self];
}
...
- (void) show
{
	...
	if ( self.style == TSAlertViewStyleInputText || self.style == TSAlertViewStyleInputNumber )
	{
        if(self.style == TSAlertViewStyleInputNumber)
        {
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	CGFloat inputRowHeight = (self.style == TSAlertViewStyleInputText || self.style == TSAlertViewStyleInputNumber) ? inputTextFieldSize.height + kTSAlertView_RowMargin : 0;
	...
	if ( layout )
	{
		...
		else
		{
            ...
            if (self.buttons.count == 2) {
                UIView *divideView = [[UIView alloc] initWithFrame:CGRectMake(self.width/2, y - 5, 1.0/[UIScreen mainScreen].scale, buttonHeight + 10)];
