...
- (void)showPropertyBar:(CGRect)frame inView:(UIView*)view viewsCanMove:(NSArray*)views
{
    ...
    if (annot) {
        ...
        if ((dvRect.origin.x + dvRect.size.width) <= 0
            ...
            || dvRect.origin.x > PDFVIEWCTRLWIDTH
            || dvRect.origin.y > PDFVIEWCTRLHEIGHT) {
            return;
        }
    }
    ...
    if (DEVICE_iPHONE) {
        ...
        if (!OS_ISVERSION8 && UIInterfaceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation)) {
            newFrame.origin.y = PDFVIEWCTRLWIDTH - newFrame.size.height;
            self.maskView.frame = CGRectMake(0, 0, PDFVIEWCTRLHEIGHT, PDFVIEWCTRLWIDTH);
        }
        else
        {
            newFrame.origin.y = PDFVIEWCTRLHEIGHT - newFrame.size.height;
            self.maskView.frame = CGRectMake(0, 0, PDFVIEWCTRLWIDTH, PDFVIEWCTRLHEIGHT);
