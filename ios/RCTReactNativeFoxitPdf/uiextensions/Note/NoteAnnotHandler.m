...
-(void)onAnnotSelected:(FSAnnot*)annot
{
    ...
    MenuItem *styleItem = [[MenuItem alloc] initWithTitle:NSLocalizedString(@"kStyle", nil) object:self action:@selector(showStyle)];
    MenuItem *deleteItem = [[MenuItem alloc] initWithTitle:NSLocalizedString(@"kDelete", nil) object:self action:@selector(deleteAnnot)];
...
- (void)showAnnotMenu
{
    ...
    if (curAnnot == self.editAnnot
        && [_extensionsManager getAnnotHandlerByAnnot:curAnnot] == self) {
