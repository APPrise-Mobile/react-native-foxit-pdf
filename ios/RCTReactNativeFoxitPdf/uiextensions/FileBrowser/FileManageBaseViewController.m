...
@interface FileManageBaseViewController () <EGORefreshTableHeaderDelegate, TSAlertViewDelegate>
...
- (BOOL)checkDiskFile:(int)currentSelectRow
{
    ...
    FbFileItem *diskFile = [[FbFileItem alloc] initWithPath:selectedFile.path modifiedDate:nil isFavorite:nil];
    if(!diskFile.isValidPDF)  //cannot find the file in this path
    {
        AlertView *alertView = [[AlertView alloc] initWithTitle:@"kWarning" message:[NSString stringWithFormat:NSLocalizedString(@"kNoFileInDisk", nil), selectedFile.fileName] buttonClickHandler:nil cancelButtonTitle:nil otherButtonTitles:@"kOK", nil];
        [alertView show];
