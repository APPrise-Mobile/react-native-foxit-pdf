...
//    [DEMO_APPDELEGATE.window addSubview:self.grayView]; //todo
    if (isZip) {
...
//    [DEMO_APPDELEGATE.window addSubview:moreTableView]; //todo
...
- (void)pushFileManageListViewWithFolder:(NSString *)folder displayName:(NSString *)displayname
{
    void(^block)() = ^(){
        ...
        {
            FbFileItem *fileItem = [[FbFileItem alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        ...
        UIImageView *imageView  = [[UIImageView alloc] initWithFrame:CGRectMake(cell.frame.size.width - 40, 0, 30, 30)];
...
- (ThumbnailScrollViewCell *)thumbnailScrollView:(ThumbnailScrollView *)scrollView cellAtIndex:(NSInteger)index
{
    ...
    if (self.viewMode == 0) //listview mode
    {
        if (cell == nil)
        {
            cell = [[ThumbnailScrollViewCell alloc] init];
