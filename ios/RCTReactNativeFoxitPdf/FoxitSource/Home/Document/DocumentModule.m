...
@property (nonatomic, strong) FbFileItem *previousItem;
...
- (instancetype)init
{
	...
	if (self)
    {
        ...
        fileBrowser.top = NO;
        TbBaseItem* documentTitle = [TbBaseItem createItemWithTitle:NSLocalizedString(@"kDocuments", nil)];
        ...
        if (DEVICE_iPHONE)
        {
            ...
            if (sortType == FileSortType_Name) {
                if (sortMode == FileSortMode_Ascending) {
                    ...
                    sortNameItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedString(@"kName", nil) imageNormal:imageNormal imageSelected:imageSelected imageDisable:nil background:nil imageTextRelation:RELATION_LEFT];
                }
                else
                {
            ...
            else if(sortType == FileSortType_Date)
            {
                if (sortMode == FileSortMode_Ascending) {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    UIImage *imageSelected = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    sortNameItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedString(@"kTimeSort", nil) imageNormal:imageNormal imageSelected:imageSelected imageDisable:nil background:nil imageTextRelation:RELATION_LEFT];
                }
                else
                {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortselect_black.png"];
            ...
            else if(sortType == FileSortType_Size)
            {
                if (sortMode == FileSortMode_Ascending) {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    UIImage *imageSelected = [UIImage imageNamed:@"document_sortupselect_black.png"];
                    sortNameItem = [TbBaseItem createItemWithImageAndTitle:NSLocalizedString(@"kSize", nil) imageNormal:imageNormal imageSelected:imageSelected imageDisable:nil background:nil imageTextRelation:RELATION_LEFT];
                }
                else
                {
                    UIImage *imageNormal = [UIImage imageNamed:@"document_sortselect_black.png"];
...
- (void)onItemCliked:(FbFileItem *)item
{
    if (!item.isFolder)
    {
        [ReactNativeFoxitPdf openPDFAtPath:item.path withPassword:nil];
        item.isOpen = YES;
    }
    else
    {
