...
@property (nonatomic, assign) enum FS_PASSWORDTYPE passwordType;
...
-(instancetype)initWithExtensionsManager:(UIExtensionsManager*)extensionsManager readFrame:(ReadFrame*)readFrame
{
	if (self = [super init]) {
        ...
        if (!group) {
            ...
            group.title = NSLocalizedString(@"kSecurity", nil);
            [_readFrame.more addGroup:group];
        }
...
- (void)tryLoadPDFDocument:(FSPDFDoc*)document guessPassword:(NSString*)guessPassword success:(void(^)(NSString* password))success error:(void(^)(NSString* description))error abort:(void(^)())abort
{
    ...
    if(status == e_errSuccess) {
        ...
        NSString* title = guessPassword ? NSLocalizedString(@"kDocPasswordError", nil) : NSLocalizedString(@"kDocNeedPassword", nil);
        [self promptForPasswordWithTitle:title callback:^(BOOL isInputed, NSString* newPassword) {
            if (isInputed) {
...
- (void)onClick:(MvMenuItem *)item
{
    ...
    if ([item.text isEqualToString:NSLocalizedString(@"kEncryption", nil)]) {
...
- (BOOL)encryptDocument:(FSPDFDoc*)doc
{
    ...
    @autoreleasepool {
        ...
        if (isOK) {
            FSPDFDoc* pdfDoc = [FSPDFDoc createFromFilePath:originalPDFPath];
...
- (BOOL)removeEncrypt:(FSPDFDoc*)pdfDoc
{
    ...
    @autoreleasepool {
        ...
        if (isOK) {
            ...
            enum FS_ERRORCODE status = [pdfDoc load:nil];
            if(status == e_errSuccess) {
                [_pdfViewCtrl setDoc:pdfDoc];
            }
        }
...
- (void)onDocOpened:(FSPDFDoc*)document error:(int)error
{
    ...
    if (self.passwordType == e_pwdUser ||
        self.passwordType == e_pwdOwner) {
        text = NSLocalizedString(@"kDecryption", nil);
    }
        ...
        if ([self.pdfDoc isModified]) { // whether to save before encryption?
            AlertView *alertView = [[AlertView alloc] initWithTitle:@"kConfirm" message:@"Do you want to save document before encryption?" buttonClickHandler:^(UIView *alertView, int buttonIndex) {
                ...
                if (!shouldSaveChanges) {
                    NSString *filePath = [ReactNativeFoxitPdf getFilePath];
                    FSPDFDoc* pdfDoc = [FSPDFDoc createFromFilePath:filePath];
                    enum FS_ERRORCODE error = [pdfDoc load:weakSelf.inputPassword];
...
- (void)handleRemoveEncryption
{
    ...
    __block void (^promptForOwnerPassword) (NSString* title, SuccessCallback success) = ^(NSString* title, SuccessCallback success) {
        [weakSelf promptForPasswordWithTitle:title callback:^(BOOL isInputed, NSString* password) {
            if (isInputed) {
                ...
                if (!isOwner) {
                    promptForOwnerPassword(NSLocalizedString(@"kDocPasswordError", nil), success);
                } else if (success) {
    ...
    void (^tryRemoveEncryption)() = ^{
        if ([weakSelf isOwner]) {
            ...
            promptForOwnerPassword(NSLocalizedString(@"kDocNeedPassword", nil), ^{
                [weakSelf removeEncryption];
            });
        }
    };
    if ([self.pdfDoc isModified]) { // whether to save before remove encryption?
        AlertView *alertView = [[AlertView alloc] initWithTitle:@"kConfirm" message:@"Do you want to save document before removing encryption?" buttonClickHandler:^(UIView *alertView, int buttonIndex) {
            BOOL shouldSaveChanges = (buttonIndex == 1);
            if (!shouldSaveChanges) {
                NSString *filePath = [ReactNativeFoxitPdf getFilePath];
                FSPDFDoc* pdfDoc = [FSPDFDoc createFromFilePath:filePath];
                NSString *password = [ReactNativeFoxitPdf getPassword];
                enum FS_ERRORCODE error = [pdfDoc load:password];
                if (error == e_errSuccess) {
                    weakSelf.pdfDoc = pdfDoc;
                } else {
                    //todo
        ...
        AlertView *alertView = [[AlertView alloc] initWithTitle:@"kConfirm" message:@"kEncryptRemovePass" buttonClickHandler:^(UIView *alertView, int buttonIndex) {
            if (buttonIndex == 0) { // no
                // nothing to do
            } else if (buttonIndex == 1) { // yes
                tryRemoveEncryption();
...
- (void)removeEncryption
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!isOK) {
                AlertView *alertView = [[AlertView alloc] initWithTitle:@"kTryAgain" message:@"kEncryptAddFail" buttonClickHandler:^(UIView *alertView, int buttonIndex) {
                    if (buttonIndex == 0) {
                        //nothing to do
                    } else if (buttonIndex == 1) { //yes
...
{
    MenuGroup * securityGroup = [_readFrame.more getGroup:TAG_GROUP_PROTECT];
