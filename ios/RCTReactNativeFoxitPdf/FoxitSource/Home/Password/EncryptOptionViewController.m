...
- (void)viewDidLoad
{
    ...
    [_buttonDone setTitle:NSLocalizedString(@"kDone", nil) forState:UIControlStateNormal];
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    if (section == 0)
    {
        return NSLocalizedString(@"kEncryptionSectionTitle", nil);
    }
    return nil;
}
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section
{
    if (section == 0)
    {
        return NSLocalizedString(@"kEncryptionSectionFooter", nil);
    }
    return nil;
}
...
- (void)initNavigationBar
{
    if (!self.navigationItem.titleView) 
    {
        ...
        titleLabel.text = NSLocalizedString(@"kEncryptionTitle", nil);
...
- (void)refreshInterface
{
    ...
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kCellEncryptOpenDocumentTitle", nil);
    }
    labelTitle = [cellPrintDoc.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kCellEncryptPrintDocumentTitle", nil);
    }
    labelTitle = [cellCopyAccessibility.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kCellEncryptCopyContentTitle", nil);
    }
    labelTitle = [_cellAnnotDoc.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kRMSANNOTATE", nil);
    }
    labelTitle = [_cellAssembleDoc.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kRMSASSEMBLE", nil);
    }
    labelTitle = [cellOpenDocPassword.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kCellEncryptPasswordTitle", nil);
    }
    ...
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kPwdDocumentLimitation", nil);
    }
    labelTitle = [_cellFillForm.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kPwdFillForm", nil);
    }
    labelTitle = [_cellEditDocument.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kPwdEditDocument", nil);
    }
    labelTitle = [_cellExtractContent.contentView viewWithTag:100];
    if ([labelTitle isMemberOfClass:[UILabel class]])
    {
        ((UILabel *)labelTitle).text = NSLocalizedString(@"kPwdExtractContent", nil);
    }
    textboxOpenDocPassword.placeholder = NSLocalizedString(@"kRequiredPlaceHolder", nil);
    textboxOtherPassword.placeholder = NSLocalizedString(@"kRequiredPlaceHolder", nil);
    [buttonEncryptRMS setTitle:NSLocalizedString(@"kRMSEncrypt", nil) forState:UIControlStateNormal];
}
