...
-(void)loadModule
{
    ...
    ReadFrame* __weak weakReadFrame = _readFrame;
    _readFrame.settingBarController.settingBar.onlytext = ^(BOOL selected) {
        [self enterReflowMode:!selected];
        weakReadFrame.hiddenSettingBar = YES;
        [weakReadFrame.settingBarController.settingBar setItemState:!selected value:0 itemType:ONLYTEXT];
    };
}
...
- (void)enterReflowMode:(BOOL)flag
{
	if (flag) {
		...
		[_readFrame changeState:STATE_REFLOW];
        ...
        [_readFrame.settingBarController.settingBar setItemState:NO value:0 itemType:ONLYTEXT];
...
{
	if ([_readFrame getState] == STATE_REFLOW) {
		if (self.viewTopReflow.hidden) {
			[self showReflowBar:YES animation:YES];
			_readFrame.isFullScreen = NO;
            [[UIApplication sharedApplication] setStatusBarHidden:NO withAnimation:YES];
		} else {
			[self showReflowBar:NO animation:YES];
			_readFrame.isFullScreen = YES;
